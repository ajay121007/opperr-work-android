 ...
 * Created by Akshay.
 */
class PayStubsFragment : BaseFragment() {

    var listDataHeader: ArrayList<String>? = null
    var mAdapter: PayStubsAdapter? = null
    ...
    override fun onStart() {
        super.onStart()
        (activity as AppCompatActivity).supportActionBar!!.title = resources.getString(R.string.tb_pay_stubs)
    }
    ...
    private fun init() {
        mAdapter = PayStubsAdapter(context!!, listDataHeader!!, listDataChild!!, object : PayStubsAdapter.ItemClick {
            override fun buttonClick(groupPosition: Int, childPosition: Int) {
                addReplaceFragment(R.id.fl_container, PayStubsPaymentsFragment(), false, true, null)
            }
        })
        el_time_sheet.setChildIndicator(null)
        el_time_sheet.setChildDivider(resources.getDrawable(R.color.white))
        el_time_sheet.setAdapter(mAdapter)
    }
