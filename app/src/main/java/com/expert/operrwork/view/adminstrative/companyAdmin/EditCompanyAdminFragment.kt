...
import com.expert.operrwork.data.model.admin.updateAdminRequest.Company
...
import com.expert.operrwork.databinding.FragmentEditCompanyAdminBinding
...
import kotlinx.android.synthetic.main.fragment_edit_company_admin.*
import kotlinx.android.synthetic.main.fragment_edit_company_admin.btn_save
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_address_city
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_address_one
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_address_two
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_address_zip
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_did
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_email
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_first_name
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_last_name
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_middle_name
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_phone
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_secondary_phone
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_state
import kotlinx.android.synthetic.main.fragment_edit_company_admin.edt_user_name
import kotlinx.android.synthetic.main.fragment_edit_company_admin.linearLayoutCompany
import kotlinx.android.synthetic.main.fragment_edit_company_admin.llBringEmployee
import kotlinx.android.synthetic.main.fragment_edit_company_admin.rv_menus
import kotlinx.android.synthetic.main.fragment_edit_company_admin.switchStatus
import kotlinx.android.synthetic.main.fragment_edit_company_admin.textStatus
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvAddAnother
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvAdminType
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvBringEmployee
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvCompany
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvGender
import kotlinx.android.synthetic.main.fragment_edit_company_admin.tvPath
 ...
 * Created by Akshay.
 */
class EditCompanyAdminFragment : BaseFragment(), View.OnClickListener {

    private lateinit var getCompanyById: GetCompanyAdminById
    private lateinit var binding: FragmentEditCompanyAdminBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater,
            R.layout.fragment_edit_company_admin,
            container,
            false
        )
        return binding.root
    ...
    private fun initObserver() {
        ...
        companyViewModel!!.getCompanyDetails().observe(this, Observer {
            if (it != null && it.status.equals("SUCCESS")) {
                binding.model = it.data
                getCompanyById = it
                selectedCompanyId = it.data?.company?.id.toString()
                selectedStaus = getCompanyById.data?.status.toString()
                tvGender.setText(getCompanyById?.data?.gender)
                filterMenuList()
                roleLevel = getCompanyById.data?.role?.level.toString()
                roleID = getCompanyById?.data?.role?.id.toString()
                tvAdminType.text = getCompanyById?.data?.role?.levelName
            } else {
                showError(getString(R.string.str_something_wrong))
            }
        })
    ...
    private fun init() {
        ...
        companyViewModel!!.getAdminById(arguments?.getString("adminId")!!)

    }

    private fun filterMenuList() {
        for (i in 0..menuItem.menu_list?.size!! - 1) {
            for (j in 0..getCompanyById.data?.menuList?.size!! - 1) {
                if (getCompanyById.data?.menuList!!.get(j).equals(menuItem.menu_list!!.get(i).id)) {
                    menuItem.menu_list!!.get(i).checked = true
                    if (!menuList.contains(getCompanyById.data?.menuList!!.get(j).toLong())) {
                        menuList.add(getCompanyById.data?.menuList!!.get(j).toLong())
                    }
                }
            }
            menuAdapter.notifyDataSetChanged()
    ...
    private fun updateAdminRequest() {
        ...
        company.id = selectedCompanyId.toInt()
        company.addressOne = edt_address_one.text.toString()
        company.addressTwo = edt_address_two.text.toString()
        company.city = edt_address_city.text.toString()
        company.state = edt_state.text.toString()
        company.zipcode = edt_address_zip.text.toString()
        ...
        updateAgencyAdminRequest.company = company
        ...
        companyViewModel?.editCompanyAdmin(updateAgencyAdminRequest)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == SELECT_FILE) run {
            var bm: Bitmap? = null
            if (data != null) {
                Glide.with(this).load(data.data).into(ivUpload)
            ...
            if (mCameraImageUri != null) {
                Glide.with(this).load(mCameraImageUri).into(ivUpload)
                try {
